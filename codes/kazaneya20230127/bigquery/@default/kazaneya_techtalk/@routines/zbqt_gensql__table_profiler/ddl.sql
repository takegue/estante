CREATE OR REPLACE FUNCTION `kazaneya_techtalk.zbqt_gensql__table_profiler`(target_table_name STRING, group_keys ARRAY<STRING>, options_json JSON)
AS (
(
with
  options as (
  ...
  , table_columns as (
    ...
    from `kazaneya_techtalk.INFORMATION_SCHEMA.COLUMNS` c
    left join `kazaneya_techtalk.INFORMATION_SCHEMA.COLUMN_FIELD_PATHS` as path
...
select
    ...
    , %s__subprofiler as (
      ...
      , agg as (
          ...
          from
            ...
            left join unnest([struct(
              generate_array(Q.qtile[offset(1)] - 1.5 * iqr, Q.qtile[offset(3)] + 1.5 * iqr, nullif(iqr * 4 / 20, 0)) as buckets
            )])
            left join unnest([struct(
  ...
  left join `kazaneya_techtalk.INFORMATION_SCHEMA.VIEWS` as path
