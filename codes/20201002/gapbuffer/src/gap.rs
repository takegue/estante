...
impl<T> GapBuffer<T> {
    ...
    /// Return the number of elements this GapBuffer cloud hold without reallocation
    ...
    /// Return a mutable pointer to ther `index` the element of underlying
    /// Safety: `index` must be a valid index into `self.storage`
    ...
    pub fn set_position(&mut self, pos: usize) {
        if pos > self.len() {
            panic!("index {} out of range for GabBuffer", pos);
        }
