...
(async() => {
  const bytes = require("fs").readFileSync("./pkg/hello_world_bg.wasm");
  const m = wasm_bindgen.initSync(bytes);
  const { greet } = m
  // console.log(wasm.greet);
  console.log(greet("hoge"));
  const manualGreet = function(name) {
          console.log('called greet');
          try {
              const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
              const ptr0 = passStringToWasm0(name, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
